...
export const load: PageServerLoad = async ({ parent, fetch }) => {
	...
	const serviceAccounts = await fetch(`/api/service_account`).then<QueriedServiceAccount[]>(
			...
			resolveRoute('/clusters/[clusterId]', { clusterId: result.cluster.id }),
			{
				...
				message: {
					title: `Created cluster "${formData.data.config.name}"${
						formData.data.nodes.length
							? ` containing ${formData.data.nodes.length} node${
									formData.data.nodes.length === 1 ? '' : 's'
								}.`
							: ''
					}`,
					description: result.message,
