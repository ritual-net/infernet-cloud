...
<button type="button" tabindex="-1" use:melt={$trigger} aria-label={labelText}>
	<slot />
</button>
...
<style>
	...
	[data-melt-tooltip-content] {
		...
		& [data-melt-tooltip-arrow] {
			filter: drop-shadow(
				calc(-1 * var(--tooltip-borderWidth)) calc(-1 * var(--tooltip-borderWidth))
					var(--tooltip-borderColor)
			);
		}
	}
</style>
