...
import { getClusterSelectParams } from './queries';
...
export async function getServiceAccountById(id: string): Promise<ProviderServiceAccount | null> {
	...
	return result ? (result as ProviderServiceAccount) : null;
}
...
export async function getClusterById(id: string, creds: boolean): Promise<ProviderCluster | null> {
	return await e
		.select(e.Cluster, () => ({
			...getClusterSelectParams(creds, { filter_single: { id } }),
		}))
		.run(client);
}
 ...
 */
export async function getClusterByNodeId(nodeId: string): Promise<ProviderCluster | null> {
	...
	const clusters = await e
		.with(
			...
			e.select(e.Cluster, (cluster) => ({
				...getClusterSelectParams(false, { filter: e.op(node, 'in', cluster.nodes) }),
			}))
		)
		.run(client);
	return clusters ? (clusters[0] as ProviderCluster) : null;
}
