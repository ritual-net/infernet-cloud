	...
	if (a.__kind__ === TypeKind.scalar && b.__kind__ === TypeKind.scalar) {
		...
		try {
			const items = a.__items__.map((_, i) => {
				...
				return getSharedParent(a.__items__[i] as SomeType, b.__items__[i] as SomeType);
			});
		...
		try {
			...
			for (const [i] of Object.entries(a.__shape__)) {
				...
				items[i] = getSharedParent(a.__shape__[i] as SomeType, b.__shape__[i] as SomeType);
			}
		...
		try {
			const mergedEl: any = getSharedParent(a.__element__ as any, b.__element__ as any);
			return {
...
type _getSharedParentPrimitiveVariadic<Types extends [any, ...any[]]> = Types extends [infer U]
	? U
	: Types extends [infer A, infer B, ...infer Rest]
		? _getSharedParentPrimitiveVariadic<[getSharedParentPrimitive<A, B>, ...Rest]>
		: never;
...
type _mergeObjectTypesVariadic<Types extends [ObjectType, ...ObjectType[]]> = Types extends [
	infer U,
]
	? U
	: Types extends [infer A, infer B, ...infer Rest]
