...
const makeGroupingSet =
	...
	<T extends SimpleGroupElements>(grps: T): { [k in keyof T]?: T[k] } => {
		...
		const unfiltered = Object.entries(grps as NestedGroupElements).flatMap(([k, grp]) =>
			isGroupingSet(grp) ? grp.__exprs__ : ([[k, grp]] as [string, SingletonSet][])
		);
				...
				ObjectType<
					'std::FreeObject',
					{
						...
						[k in keyof Mods['by']]: Mods['by'][k]['__element__'] extends ObjectType
							? never
							: PropertyDesc<Mods['by'][k]['__element__'], Cardinality.AtMostOne>;
					}
				>,
				Cardinality.One,
				{},
...
type groupFunc = <
	...
	Shape extends { by?: SimpleGroupElements } & objectTypeToSelectShape<Expr['__element__']>,
	// Mods extends GroupModifiers = {by: Shape["by"]}
>(
	expr: Expr,
	getter: (arg: $scopify<Expr['__element__']>) => Readonly<Shape>
) => $expr_Group<Expr, { by: noUndefined<Shape['by']> }, normaliseShape<Shape, 'by'>>;
