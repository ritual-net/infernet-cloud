...
function applySpec(
	...
	const allPointers = [...type.pointers, ...type.backlinks, ...type.backlink_stubs];
		...
		if (ptr.kind === 'link') {
			...
			util.defineGetter(shape[ptr.name], 'properties', () => {
				if (!shape[ptr.name][_linkProps]) {
					const linkProperties: { [k: string]: any } = (shape[ptr.name][_linkProps] = {});
...
export function makeType<T extends BaseType>(
	...
	if (type.kind === 'object') {
		...
		if (type.tuple_elements[0]!.name === '0') {
			...
			util.defineGetter(obj, '__name__', () => {
				return `tuple<${obj.__items__.map((item: any) => item.__name__).join(', ')}>`;
			});
			return obj;
		} else {
			// named tuple
...
export function $mergeTupleTypes<A extends TupleType, B extends TupleType>(a: A, b: B): TupleType {
