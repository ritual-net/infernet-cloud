...
export function $resolveOverload(
	...
	throw new Error(
		`No function overload found for ${
			funcName.includes('::') ? `'e.${funcName.split('::').join('.')}()'` : `operator '${funcName}'`
...
function _tryOverload(
	...
	for (let i = 0; i < funcDef.args.length; i++) {
		...
		if (arg === undefined) {
			...
			positionalArgs.push(...(argDef.variadic ? (args.slice(i) as BaseTypeSet[]) : [arg]));
			if (argDef.setoftype) {
				paramCardinalities.push(
					funcDef.preservesOptionality
				...
				const card = argDef.variadic
					? cardutil.multiplyCardinalitiesVariadic(
							(args.slice(i) as BaseTypeSet[]).map((el) => el.__cardinality__) as [
								Cardinality,
								...Cardinality[],
							]
						)
	...
	if (type.kind === 'scalar') {
		...
		return {
			match:
				...
				(arg.__name__ === type.name || isImplicitlyCastableTo(arg.__name__, type.name)),
		};
	}
	if (type.kind === 'array') {
