	...
	: {
			[k in keyof Shape & string]: Shape[k] extends ObjectTypeSet
				? $expr_PathNode<
						TypeSet<
							Shape[k]['__element__'],
							cardutil.multiplyCardinalities<Root['__cardinality__'], Shape[k]['__cardinality__']>
						>,
...
> = {
	[k in keyof Props & string]: Props[k] extends PropertyDesc
		? $expr_PathLeaf<
				TypeSet<
					Props[k]['target'],
					cardutil.multiplyCardinalities<Root['__cardinality__'], Props[k]['cardinality']>
				>,
	...
	get(target: any, prop: string | symbol, proxy: any) {
		...
		if (ptr) {
			return (
				...
				(target[_pathCache][prop] = ((ptr.__kind__ === 'property' ? PathLeaf : PathNode) as any)(
					{
...
export function $expressionify<T extends ExpressionRoot>(_expr: T): Expression<T> {
...
export function $getScopedExpr<T extends ExpressionRoot>(
	...
	if (!scopedExpr || existingScopes?.has(scopedExpr)) {
		...
		const isFreeObject =
			expr.__cardinality__ === Cardinality.One && expr.__element__.__name__ === 'std::FreeObject';
		...
		scopedExpr = isFreeObject
			...
			: $expressionify({
					...
					...(expr.__element__.__kind__ === TypeKind.object
						? {
								'*': getStarShapeFromPointers((expr.__element__ as ObjectType).__pointers__),
							}
						: {}),
				});
