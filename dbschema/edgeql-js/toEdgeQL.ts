...
export function $toEdgeQL(this: any) {
	...
	for (const [expr, refData] of seen) {
		...
		if (
			...
			const scopeVarName = `__scope_${withVars.size}_${scopeVar.__element__.__name__.replace(
				/[^A-Za-z]/g,
				''
			)}`;
			...
			withVars.set(scopeVar, {
				...
				childExprs: new Set(),
				scopedExpr:
					expr.__element__.__kind__ === TypeKind.object ? (expr.__expr__ as any) : undefined,
			});
		}
		...
		if (refData.refCount > 1 || refData.boundScope || refData.aliases.length > 0) {
			...
			if (!withBlock) {
				...
				const [firstParentScopeChain, ...parentScopeChains] = parentScopes.map((scope) => {
					...
					while (pendingScopes.length) {
						...
						pendingScopes.push(
							...[...walkExprCtx.seen.get(currentScope)!.parentScopes].filter((s) => s !== null)
						);
			...
			const validScopes = new Set([withBlock, ...walkExprCtx.seen.get(withBlock)!.childExprs]);
			for (const scope of [
				...refData.parentScopes,
				...util.flatMap(refData.aliases, (alias) => [...walkExprCtx.seen.get(alias)!.parentScopes]),
			]) {
		...
		case ExpressionKind.Update: {
			...
			if (expr.__kind__ === ExpressionKind.Select) {
				if (
					...
					expr.__element__.__shape__ !== (expr.__expr__ as ObjectTypeSet).__element__.__shape__
				) {
		...
		case ExpressionKind.InsertUnlessConflict: {
			...
			if (expr.__conflict__.on) {
				insertChildExprs.push(
					...walkExprTree(expr.__conflict__.on, expr.__expr__ as $expr_Insert, ctx)
				);
			}
			if (expr.__conflict__.else) {
				insertChildExprs.push(
					...walkExprTree(expr.__conflict__.else, expr.__expr__ as $expr_Insert, ctx)
				);
			}

			walkExprTree(expr.__expr__, parentScope, ctx);
			ctx.seen.get(expr.__expr__ as $expr_Insert)!.childExprs.push(...insertChildExprs);
			break;
		}
...
function renderEdgeQL(
	...
	if (withVar && ctx.renderWithVar !== expr) {
		return renderShape && expr.__kind__ === ExpressionKind.Select && isObjectType(expr.__element__)
	...
	function renderWithBlockExpr(varExpr: SomeExpression, _noImplicitDetached?: boolean) {
		...
		return `  ${withBlockElement.name} := ${
			renderedExpr.includes('\n')
				? `(\n${indent(
						renderedExpr[0] === '(' && renderedExpr[renderedExpr.length - 1] === ')'
	...
	if (ctx.withBlocks.has(expr as any) || scopeExpr) {
		...
		const sortedBlockVars = topoSortWithVars(ctx.withBlocks.get(expr as any) ?? new Set(), ctx);
	...
	const withBlockElements = [...unscopedWithBlock, ...scopeExprVar, ...scopedWithBlock];
	...
	if (expr.__kind__ === ExpressionKind.With) {
		...
		if (!expr.__parent__) {
			...
			const isScopedLinkProp =
				expr.__parent__.linkName.startsWith('@') && ctx.withVars.has(expr.__parent__.type as any);
		...
		return `(\n${Object.keys(expr.__shape__)
			.map(
				(key) =>
					`  ${key} := ${renderEdgeQL(expr.__shape__[key]!, ctx, renderShape, noImplicitDetached)}`
			)
			.join(',\n')}\n)`;
		...
		const typeName =
			expr.__element__.__name__ === 'std::number' ? 'std::float64' : expr.__element__.__name__;
		...
		if (isObjectType(expr.__element__)) {
			...
			if (expr.__element__.__shape__ !== (expr.__expr__ as ObjectTypeSet).__element__.__shape__) {
				lines.push(
					shapeToEdgeQL((expr.__element__.__shape__ || {}) as object, ctx, expr.__element__)
				);
			}
		} else {
		...
		if (expr.__modifiers__.order_by) {
			modifiers.push(
				...expr.__modifiers__.order_by.map(({ expression, direction, empty }, i) => {
					return `${i === 0 ? 'ORDER BY' : '  THEN'} ${renderEdgeQL(expression, ctx)}${
						direction ? ' ' + direction : ''
					}${empty ? ' ' + empty : ''}`;
				})
			);
		...
		if (expr.__modifiers__.offset) {
			modifiers.push(`OFFSET ${renderEdgeQL(expr.__modifiers__.offset as OffsetExpression, ctx)}`);
		}
		if (expr.__modifiers__.limit) {
			modifiers.push(`LIMIT ${renderEdgeQL(expr.__modifiers__.limit as LimitExpression, ctx)}`);
		}
		...
		return `(${withBlock}INSERT ${renderEdgeQL(expr.__expr__, ctx, false, true)} ${shapeToEdgeQL(
			expr.__shape__,
			ctx,
			null,
			false,
...
${indent(groupStatement.join('\n'), 4)}
  ...
  elements: ${elementsShapeQuery}
		...
		return `(${withBlock}FOR ${ctx.forVars.get(expr.__forVar__)} IN {${renderEdgeQL(
			expr.__iterSet__,
			ctx
		)}}
UNION (\n${indent(renderEdgeQL(expr.__expr__, ctx), 2)}\n))`;
		...
		util.assertNever(expr, new Error(`Unrecognized expression kind: "${(expr as any).__kind__}"`));
	}
}
	...
	for (const key in shape) {
		...
		const expectedCardinality =
			addCardinalityAnnotations && val.hasOwnProperty('__cardinality__')
				? val.__cardinality__ === Cardinality.Many || val.__cardinality__ === Cardinality.AtLeastOne
					? 'multi '
					: 'single '
				: '';
...
function topoSortWithVars(vars: Set<SomeExpression>, ctx: RenderCtx): SomeExpression[] {
	if (!vars.size) {
		return [];
	}
...
function literalToEdgeQL(type: BaseType, val: any): string {
	...
	if (typename === 'std::json') {
		...
		if (isArrayType(type)) {
			stringRep = `[${val.map((el) => literalToEdgeQL(type.__element__ as any, el)).join(', ')}]`;
		...
		return `std::range(${
			val.lower === null ? `<${elTypeName}>{}` : literalToEdgeQL(elType, val.lower)
		}, ${
			val.upper === null ? `<${elTypeName}>{}` : literalToEdgeQL(elType, val.upper)
...
function q(ident: string, allowBacklinks: boolean = true): string {
	...
	if (isAlphaNum) {
		...
		const isReserved =
			lident !== '__type__' && lident !== '__std__' && reservedKeywords.has(lident);
