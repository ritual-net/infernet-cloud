...
import { Cardinality, ExpressionKind, type typeutil, TypeKind } from 'edgedb/dist/reflection/index';
import type {
	Expression,
	LinkDesc,
	ObjectTypeSet,
			...
			? Shape extends ObjectTypePointers
				? typeutil.addQuestionMarks<{
						[k in keyof Shape]:
							...
							| (pointerIsOptional<Shape[k]> extends true ? undefined | null : never)
...
function unlessConflict(this: $expr_Insert, conflictGetter?: (scope: TypeSet) => UnlessConflict) {
	const expr: any = {
		__kind__: ExpressionKind.InsertUnlessConflict,
...
export function $insertify(expr: Omit<$expr_Insert, 'unlessConflict'>): $expr_Insert {
...
export function $normaliseInsertShape(
	...
	const _shape: [string, any][] =
		shape.__element__?.__kind__ === TypeKind.namedtuple
			? Object.keys((shape.__element__ as NamedTupleType).__shape__).map((key) => [key, shape[key]])
	...
	for (const [key, _val] of _shape) {
		...
		const isMulti =
			pointer.cardinality === Cardinality.AtLeastOne || pointer.cardinality === Cardinality.Many;
		...
		newShape[key] = setModify ? ({ [setModify]: wrappedVal } as any) : wrappedVal;
	}
	return newShape;
}
