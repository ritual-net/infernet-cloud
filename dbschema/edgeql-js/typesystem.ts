...
export type Expression<Set extends TypeSet = TypeSet, Runnable extends boolean = true> = Set &
...
export type ExpressionMethods<Set extends TypeSet> = {
	...
	is<T extends ObjectTypeSet>(
		...
		ObjectType<T['__element__']['__name__'], T['__element__']['__pointers__'], { id: true }>
	>;
...
export type omitBacklinks<T extends string | number | symbol> = T extends `<${string}`
	? never
	: T extends string
		? T
		: never;
...
type shapeElementToTs<Pointer extends PropertyDesc | LinkDesc, Element> = [Element] extends [true]
...
export type computeObjectShape<Pointers extends ObjectTypePointers, Shape> = typeutil.flatten<
	keyof Shape extends never
		? { id: string }
		: {
				[k in keyof Shape]: Shape[k] extends $expr_PolyShapeElement<infer PolyType, infer ShapeEl>
					? [k] extends [keyof PolyType['__element__']['__pointers__']]
						? shapeElementToTs<PolyType['__element__']['__pointers__'][k], ShapeEl> | null
						: never
...
export type pointerToTsTypeSimple<El extends PropertyDesc | LinkDesc> = El extends PropertyDesc
	? propToTsType<El>
	...
	? {
			[index: number]: $expr_Operator<
				...
				getPrimitiveBaseType<
					Set['__element__'] extends ArrayType<infer El> ? El : Set['__element__']
				>,
				Set['__cardinality__']
				// >
			>;
			...
			>(
				...
				// >
			>;
			slice<E extends TypeSet<ScalarType<'std::number'>> | number | undefined | null>(
...
type ArrayTypeToTsType<Type extends ArrayType, isParam extends boolean = false> = BaseTypeToTsType<
	Type['__element__'],
	isParam
>[];
...
export type tupleElementsToCardTuple<T> = baseTupleElementsToCardTuple<T> extends [
	Cardinality,
	...Cardinality[],
]
...
export type $expr_Tuple<Items extends typeutil.tupleOf<TypeSet> = typeutil.tupleOf<TypeSet>> =
	Expression<{
...
type inferNamedTupleCardinality<Shape extends NamedTupleLiteralShape> = [Cardinality.Many] extends [
	shapeCardinalities<Shape>,
]
	? Cardinality.Many
	: [Cardinality.Empty] extends [shapeCardinalities<Shape>]
		...
		: [shapeCardinalities<Shape>] extends [Cardinality.AtMostOne]
			? Cardinality.AtMostOne
			: [shapeCardinalities<Shape>] extends [Cardinality.AtMostOne | Cardinality.One]
				? Cardinality.One
				: Cardinality.Many;

export type $expr_NamedTuple<Shape extends NamedTupleLiteralShape = NamedTupleLiteralShape> =
	Expression<{
					...
					: Type extends TupleType
						...
						: Type extends NamedTupleType
							...
							: Type extends ObjectType
								? typeutil.flatten<computeObjectShape<Type['__pointers__'], Type['__shape__']>>
								: never;
...
export type computeTsTypeCard<T extends any, C extends Cardinality> = Cardinality extends C
	? unknown
...
export type computeTsType<T extends BaseType, C extends Cardinality> = BaseType extends T
	? unknown
	: computeTsTypeCard<BaseTypeToTsType<T>, C>;
...
export type pointerToTsType<El extends PropertyDesc | LinkDesc> = El extends PropertyDesc
	? propToTsType<El>
	: El extends LinkDesc<any, any, any, any>
		? linkToTsType<El>
		: never;
...
export type getPrimitiveNonArrayBaseType<T extends BaseType> = T extends ArrayType
	? never
	: getPrimitiveBaseType<T>;
