...
import { Cardinality, ExpressionKind, TypeKind, OperatorKind } from 'edgedb/dist/reflection/index';
...
import { type scalarLiterals, type literalToScalarType, literalToTypeSet } from './castMaps';
		...
		: {
				[j in keyof E]: {
					[k in keyof E[j]]: E[j][k] extends ObjectTypeSet
						? TypeSet<anonymizeObject<E[j][k]['__element__']>, E[j][k]['__cardinality__']>
...
export interface SelectModifierMethods<Root extends TypeSet> {
	filter<Filter extends SelectFilterExpression>(
		filter:
			| Filter
			| ((
					scope: Root extends ObjectTypeSet ? $scopify<Root['__element__']> : stripSet<Root>
			  ) => Filter)
	): this;
	order_by(
		order_by:
...
>(
	...
	[k in Exclude<keyof Shape, SelectModifierNames | 'id'>]: $expr_PolyShapeElement<
		Expr,
...
export function $handleModifiers(
	...
	if (modifiers.order_by) {
		...
		mods.order_by = orderExprs.map((expr) =>
			typeof (expr as any).__element__ === 'undefined' ? expr : { expression: expr }
		) as any;
	}
	if (modifiers.offset) {
		mods.offset =
	...
	if (modifiers.limit) {
		...
		if (typeof modifiers.limit === 'number') {
			...
			const type = (modifiers.limit.__element__ as any).__casttype__ ?? modifiers.limit.__element__;
...
export type linkDescToLinkProps<Desc extends LinkDesc> = {
	[k in keyof Desc['properties'] & string]: $expr_PathLeaf<
		TypeSet<Desc['properties'][k]['target'], Desc['properties'][k]['cardinality']>
		// {
...
export const $existingScopes = new Set<Expression<TypeSet<BaseType, Cardinality>>>();

function $shape<
	Expr extends ObjectTypeExpression,
	Shape extends objectTypeToSelectShape<Expr['__element__']> & SelectModifiers<Expr['__element__']>, // <Expr["__element__"]>
>(
	expr: Expr,
	_shape: (
...
export function select<Expr extends TypeSet>(expr: Expr): $expr_Select<stripSet<Expr>>;
...
export function select<Expr extends PrimitiveTypeSet, Modifiers extends SelectModifiers>(
	expr: Expr,
...
export function select(...args: any[]) {
	...
	const exprPair: [TypeSet, (scope: any) => any] =
		typeof args[0].__element__ !== 'undefined' ? (args as any) : [FreeObject, () => args[0]];
...
export function resolveShapeElement(key: any, value: any, scope: ObjectTypeExpression): any {
	...
	const isClosure =
		typeof value === 'function' && scope.__element__.__pointers__[key]?.__kind__ === 'link';
	// if (isSubshape) {
	//   // return value;
