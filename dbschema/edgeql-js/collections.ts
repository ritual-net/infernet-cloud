...
import { Cardinality, ExpressionKind, TypeKind, type typeutil } from 'edgedb/dist/reflection/index';
			...
			return $expressionify({
				...
				__args__: [
					...
					isIndex
						...
						: [start && literalToTypeSet(Number(start)), end && literalToTypeSet(Number(end))],
				],
			}) as any;
		}
...
export function $arrayLikeIndexify(_expr: ExpressionRoot) {
	if (
		...
		(_expr.__element__.__kind__ === TypeKind.scalar &&
			(_expr.__element__.__name__ === 'std::str' || _expr.__element__.__name__ === 'std::bytes'))
	) {
...
// ARRAY
export function array<Element extends NonArrayType>(element: Element): ArrayType<Element>;
...
export function tuple<Items extends typeutil.tupleOf<BaseType>>(items: Items): TupleType<Items>;
...
export function tuple<Shape extends NamedTupleShape>(shape: Shape): NamedTupleType<Shape>;
...
export function $objectTypeToTupleType(...args: any[]): any {
	const [objExpr, fields] = args as [ObjectTypeExpression, string[] | undefined];
